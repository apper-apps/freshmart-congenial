...
const selectedSlot = deliverySlots.find(slot => slot.value === deliveryInfo.deliverySlot);
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
            <h1 className="text-3xl font-display font-bold text-gray-900 mb-2">Checkout
                          </h1>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="bg-white rounded-xl shadow-premium p-6">
                        <h2 className="font-display font-semibold text-xl text-gray-900 mb-6">
                            <ApperIcon name="MapPin" className="w-5 h-5 inline mr-2" />Delivery Information
                                            </h2>
                    ...
                    <div className="bg-white rounded-xl shadow-premium p-6">
                        <h2 className="font-display font-semibold text-xl text-gray-900 mb-6">
                            <ApperIcon name="Clock" className="w-5 h-5 inline mr-2" />Delivery Time
                                            </h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {deliverySlots.map(slot => <label
                                key={slot.value}
                                className={`p-4 rounded-lg border-2 cursor-pointer transition-all duration-200 ${deliveryInfo.deliverySlot === slot.value ? "border-primary-500 bg-primary-50" : "border-gray-200 bg-white hover:border-gray-300"}`}>
                                <input
                                    type="radio"
                                    name="deliverySlot"
                                    value={slot.value}
...
<div>
                    ...
                    <div className="bg-white rounded-xl shadow-premium p-6">
                        <h2 className="font-display font-semibold text-xl text-gray-900 mb-6">
                            <ApperIcon name="CreditCard" className="w-5 h-5 inline mr-2" />Payment Method
                                            </h2>
                        {gatewaysLoading ? <div className="flex items-center justify-center py-8">
                            <div
                                className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-500"></div>
                            <span className="ml-3 text-gray-600">Loading payment methods...</span>
                        </div> : availableGateways.length === 0 ? <div className="text-center py-8">
                            ...
                            {availableGateways.map(gateway => <label
                                key={gateway.Id}
                                className={`flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all duration-200 ${paymentMethod === gateway.Id.toString() ? "border-primary-500 bg-primary-50" : "border-gray-200 bg-white hover:border-gray-300"}`}>
                                <input
                                    type="radio"
                                    name="paymentMethod"
                                ...
                                {paymentMethod === gateway.Id.toString() && <ApperIcon name="CheckCircle" className="w-5 h-5 text-primary-500" />}
                            </label>)}
                        </div>}
                    </div>
                </div>
                {/* Transaction Validation Notice */}
                {selectedGateway && <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                    <div className="flex items-start space-x-3">
                        <ApperIcon name="Shield" className="w-5 h-5 text-blue-600 mt-0.5" />
                        <div>
                            <h3 className="font-medium text-blue-900 mb-2">Automated Transaction Validation</h3>
                            <p className="text-sm text-blue-800 mb-3">Your payment will be automatically validated using our comprehensive verification system:
                                                      </p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                <div className="flex items-center space-x-2">
                                    <ApperIcon name="Building" className="w-4 h-4 text-blue-600" />
                                    <span className="text-sm text-blue-800">Bank reference cross-check</span>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <ApperIcon name="Clock" className="w-4 h-4 text-blue-600" />
                                    <span className="text-sm text-blue-800">Payment timestamp verification</span>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <ApperIcon name="DollarSign" className="w-4 h-4 text-blue-600" />
                                    <span className="text-sm text-blue-800">Automated amount matching</span>
                                </div>
                                <div className="flex items-center space-x-2">
                                    <ApperIcon name="Wifi" className="w-4 h-4 text-blue-600" />
                                    <span className="text-sm text-blue-800">Third-party processor integration</span>
                                </div>
                            </div>
                            <div className="mt-3 p-3 bg-blue-100 rounded-lg">
                                <p className="text-xs text-blue-700">
                                    <ApperIcon name="Info" className="w-3 h-3 inline mr-1" />Validation typically completes within 30 seconds of payment confirmation.
                                                              You'll receive instant notification once verified.
                                                            </p>
                            </div>
                        </div>
                    </div>
...
<div className="space-y-3 mb-6">
                    ...
                    <Button
                        ...
                        <ApperIcon name="ShoppingBag" className="w-5 h-5 mr-2" />Place Order
                                        </Button>
                </div>
            </div>
        </form></div>
